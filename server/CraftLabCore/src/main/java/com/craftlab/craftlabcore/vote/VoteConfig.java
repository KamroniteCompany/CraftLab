...
/**
 * Lit config/craftlabcore/config.properties (créé avec une valeur par défaut s'il n'existe pas).
 * VoteManager relit cette config à chaque /mod start &lt;modId&gt;, donc changer la valeur
    ...
    private static void writeDefault(Path configFile) {
        ...
        try {
            ...
            try (OutputStream out = Files.newOutputStream(configFile)) {
                properties.store(out, "Duree d'un vote CraftLabCore, en secondes. 604800 = 7 jours. Ex : 60 pour un vote de test d'une minute.");
            }
