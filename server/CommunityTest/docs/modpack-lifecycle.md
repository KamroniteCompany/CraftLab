...
démarrage de la JVM**, avant que le code de nos propres mods (dont CommunityTest, qui est
lui-même chargé *depuis* `/mods`) ne s'exécute. Le jeu des mods actifs pour un démarrage
donné est donc déjà figé bien avant qu'un `@SubscribeEvent` de CommunityTest ne se
déclenche.
...
est d'abord copié dans `config/communitytest/staging/` (jamais dans `downloads/`, qui
...
`config/communitytest/backups/<horodatage>/` : une copie de chaque fichier actuellement
...
Les fichiers de `config/communitytest/downloads/` (le cache de téléchargement) ne sont
...
`ModPackApplier` écrit un marqueur (`config/communitytest/modpack/apply-state.txt`)
...
`config/communitytest/managed-mods.json` liste précisément quels fichiers de `/mods` sont
